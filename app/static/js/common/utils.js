 ...
 * 标准表格生成器 - 使用统一的表格样式
 */
export const StandardTableGenerator = {
    /**
     * 生成标准表格HTML
     * @param {Object} config - 表格配置
     * @param {string} config.id - 表格ID
     * @param {Array} config.headers - 表头数组
     * @param {Array} config.data - 数据数组
     * @param {Function} config.rowRenderer - 行渲染函数
     * @param {string} config.noDataMessage - 无数据提示
     * @returns {string} 表格HTML
     */
    generateTable(config) {
        const { id, headers, data, rowRenderer, noDataMessage = "没有找到匹配的记录" } = config;

        if (!data || data.length === 0) {
            return `<div class="text-muted text-center py-3 small">${noDataMessage}</div>`;
        }

        return `
            <div class="table-responsive">
                <table class="table table-hover table-sm mb-0 standard-table" id="${id}">

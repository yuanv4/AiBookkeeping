...
def _update_merchant_category_internal(merchant_name: str, category: str, operation_name: str):
    ...
    try:
        ...
        logger.info(f"{operation_name}: {merchant_name} -> {category}, 影响 {updated_count} 笔交易")
        ...
        db.session.rollback()
        logger.error(f"{operation_name}失败: {merchant_name} -> {category}, 错误: {e}")
...
def get_uncategorized_merchants():
    ...
    for merchant in merchants_data:
        # 从批量结果中获取AI建议
        ai_suggestion = ai_suggestions.get(merchant.merchant_name) or {
            'category': 'uncategorized', 'category_name': '未分类', 'confidence': 0
        }

        merchants_list.append({
            'id': abs(hash(merchant.merchant_name)) % 999999,
...
def confirm_ai_suggestion():
    ...
    return _update_merchant_category_internal(merchant_name, category, 'AI建议确认')

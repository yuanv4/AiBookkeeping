...
class FinancialService:
    ...
    # ==================== 从 TransactionService 迁移的分析方法 ====================
    
    def get_summary_by_type(self, account_id: Optional[int] = None, 
        ...
        try:
            ...
            transfer_query = self.db.query(
                func.literal('transfer').label('type'),
                func.count(Transaction.id).label('count'),
                func.sum(Transaction.amount).label('total')
            ).filter(Transaction.amount == 0)
            ...
            self.logger.error(f"Error getting summary by type: {e}")
            return []
    
    def get_monthly_summary(self, account_id: Optional[int] = None, year: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            results = query.group_by(func.extract('month', Transaction.date)).order_by('month').all()
    ...
    def get_expense_statistics_legacy(self, start_date: Optional[date] = None, 
                             end_date: Optional[date] = None, 
                             account_id: Optional[int] = None) -> Dict[str, Any]:
        """获取支出统计信息（从TransactionService迁移）
        ...
        try:
            ...
            self.logger.error(f"Error getting expense statistics: {e}")
            return {}

    def get_top_expense_counterparties_legacy(self, start_date: Optional[date] = None, 
                                     end_date: Optional[date] = None, 
                                     account_id: Optional[int] = None, 
                                     limit: int = 10, 
                                     sort_by: str = 'amount') -> List[Dict[str, Any]]:
        """获取支出对手方排名（从TransactionService迁移）
    ...
    def get_expense_transactions_paginated(self, filters: Dict[str, Any] = None, 
                                         page: int = 1, 
                                         per_page: int = 20):
        """获取支出交易记录（分页）（从TransactionService迁移）
        ...
        try:
            ...
            self.logger.error(f"Error getting expense transactions with pagination: {e}")
            # 返回空的分页对象
            return self.db.query(Transaction).filter(Transaction.id == -1).paginate(
                page=1, per_page=per_page, error_out=False
            )

    def get_total_amount_legacy(self, start_date: date, end_date: date, 
                        account_id: Optional[int] = None, 
                        transaction_type: str = 'all') -> float:
        """获取总金额（从TransactionService迁移）"""
        try:
            query = self.db.query(Transaction).filter(
                Transaction.date >= start_date,
                Transaction.date <= end_date

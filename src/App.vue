  ...
  <div class="container">
    ...
    <div class="card">
      ...
      >
        ...
        <div class="upload-hint">支持微信账单(xlsx)、支付宝账单(csv)、银行账单(xls/pdf)</div>
        <input
          ref="fileInput"
          type="file"
          multiple
      ...
      <div v-if="files.length > 0" class="file-list">
        <div v-for="file in files" :key="file.id" class="file-item">
          ...
          <button class="btn btn-danger" @click="removeFile(file.id)">删除</button>
        </div>
      </div>
    ...
    <div v-if="transactions.length > 0" class="summary-cards">
      ...
      <div class="summary-card expense">
        ...
        <div class="value">¥{{ Math.abs(statistics.expense).toFixed(2) }}</div>
      </div>
      <div class="summary-card">
        <div class="icon">📊</div>
    ...
    <div v-if="transactions.length > 0" class="card">
      <div class="filters">
        ...
        <button class="btn btn-success" @click="exportTransactions">导出Excel</button>
      </div>
    </div>
...
async function processFiles() {
  ...
  try {
    ...
    // 合并和去重
    transactions.value = deduplicateTransactions(allTransactions)
  } catch (error) {
...
function exportTransactions() {
  ...
  const exportData = filteredTransactions.value.map(t => ({
    '交易时间': formatDateTime(t.transactionTime),
    '平台': getPlatformLabel(t.platform, t.bankName),
...
// 监听筛选器
;[searchQuery, filterPlatform, filterType].forEach(ref => {
